...
string filename_index = "DekobokoR"; //ファイルの名前
int file_number = 0; //ファイル番号
int total_pieces = 4; //ピース数
double grobal_width = 0;
double grobal_height = 500;
double r1[2][2];
string readfilename = "Data1.CSV";
...
int main(void) {
	...
	for (int aa = 0; aa < 20; aa++) {
		...
		for (int s = 0; (int)s < total_pieces - 1; s++) {
			...
			currentPieceID.push_back(pair2.pieceID);
		}
		write_pieces_vertex(fixedPiece);
		cout << "aaaaaa" << endl;
	}
...
double calcDegree(double a1, double a2, double b1, double b2) {
	...
	return acos(cosDeg) * 180 / 3.141592653589;
}
...
//頂点の内外判定////////////////////////////////////////////////////////////////////////////////////////////////////
bool InputCheck(pieces p1, pieces p2) {
	double deg = 0, inner_pro, count_deg = 0, pi = 3.141592653589;
